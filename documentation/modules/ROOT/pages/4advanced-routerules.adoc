...
Open the url Customer Gateway in your broswer 

[source,bash,subs="+macros,+attributes"]
----
export INGRESS_PORT=$(kubectl -n istio-system get service istio-ingressgateway -o jsonpath='{.spec.ports[?(@.name=="http2")].nodePort}')

open http://$(minikube ip):$INGRESS_PORT/customer
----
...
[source,bash,subs="+macros,+attributes"]
----
curl -A Safari http://$(minikube ip):$INGRESS_PORT/{path}
curl -A Firefox http://$(minikube ip):$INGRESS_PORT/{path}
----
