...
to Istio  configuration (besides the ones required to install
Istio).
...
A prerequisite for installing Kiali is that you must have Istio
...
So now we can access Kiali at `http://localhost:20001/kiali/console`, so
let's do it:
